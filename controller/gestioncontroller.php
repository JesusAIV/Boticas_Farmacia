...
class gestionController extends gestionModel
{
    ...
    public function paginationProducts()
    {
        ...
        $totalRegistros = gestionModel::contarProductsM();
        $totalPaginas = ceil($totalRegistros / $cantidad);
        ...
        $registros =  gestionModel::obtenerProductosM($inicio, $cantidad);

        $tabla = '';

        $tabla .= '
                <div class="table">
                    <table class="table_usuarios" id="table_productos">
                        <thead class="thead_usuarios">
                            <tr class="tr-thead_usuarios">
                                <th class="th_usuarios">ID</th>
                                <th class="th_usuarios">Nombre</th>
                                <th class="th_usuarios">Categoria</th>
                                <th class="th_usuarios">Precio</th>
                                <th class="th_usuarios">Stock</th>
                                <th class="th_usuarios">Vencimiento</th>
                                <th class="th_usuarios">Proveedor</th>
                                <th class="th_usuarios">Codigo de barra</th>
                                <th class="th_usuarios">Imagen</th>
                                <th class="th_usuarios">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="tbody_usuarios">
        ...
        if (!empty($registros)) {
            foreach ($registros as $registro) {
                $tabla .= '
                        <tr class="tr-tbody_usuarios" data-id="' . $registro['id'] . '">
                            <td class="td_usuarios">' . $registro['id'] . '</td>
                            <td class="td_usuarios">' . $registro['name'] . '</td>
                            <td class="td_usuarios">' . $registro['category'] . '</td>
                            <td class="td_usuarios">' . $registro['price'] . '</td>
                            <td class="td_usuarios">' . $registro['stock'] . '</td>
                            <td class="td_usuarios">' . $registro['dueDate'] . '</td>
                            <td class="td_usuarios">' . $registro['provider'] . '</td>
                            <td class="td_usuarios">' . $registro['barcode'] . '</td>
                            <td class="td_usuarios">' . $registro['image'] . '</td>
                            <td class="td_usuarios td-acciones_usuarios">
                                <button class="accion accion-ver_usuarios">Ver</button>
                                <button class="accion accion-editar_usuarios">Editar</button>
                                <button class="accion accion-eliminar_usuarios">Eliminar</button>
            ...
            if ($pagina > 1) {
                $tabla .= '
                    <div class="pagination_control pagination_previous">
                        <a href="' . SERVERURL . 'gestion/almacen?pagina=' . ($pagina - 1) . '" class="previous">Anterior</a>
                    </div>
                ';
            }
            ...
            for ($i = 1; $i <= $totalPaginas; $i++) {
                if ($i == $pagina) {
                    $tabla .= '<a href="' . SERVERURL . 'gestion/almacen?pagina=' . $i . '" class="number number_active">' . $i . '</a>';
                } else {
                    $tabla .= '<a href="' . SERVERURL . 'gestion/almacen?pagina=' . $i . '" class="number">' . $i . '</a>';
                }
            }
            ...
            if ($pagina < $totalPaginas) {
                $tabla .= '
                    <div class="pagination_control pagination_next">
                        <a href="' . SERVERURL . 'gestion/almacen?pagina=' . ($pagina + 1) . '" class="next">Siguiente</a>
                    </div>
                ';
            }
            ...
            $tabla .= '
                    <tr class="tr-tbody_usuarios">
                        <td class="td_usuarios" colspan="10">
                        No hay registros
                        <a href="' . SERVERURL . 'gestion/almacen?pagina=' . (1) . '" class="error_registro">Ir a pagina 1</a>
                        </td>
                    </tr>
                ';
        }
     ...
     * @return array Datos del usuario
     */
    public function ProductByIdC($idProduct)
    {
        $datosUser = gestionModel::ProductByIdM($idProduct);
